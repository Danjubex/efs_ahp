...
data_path = "exp1.csv"  # Substitua pelo caminho correto
...
custom_order = [
    ...
    "LASSO", "TSNE", "UMAP",
    "PCA", "RANDOM SEARCH",  "LAPPAS & YANNACOPOULOS", "[EFS-AHP]GPT", "[EFS-AHP]DEC 1", "[EFS-AHP]DEC 2", "[EFS-AHP]DEC 3", "[EFS-AHP]GRUPO"
]
...
for column in numeric_columns:
    ...
    if follows_normal:
        ...
        comparison_matrix_ttest.to_csv(f"[EXP1]ttest_{column}.csv")
    else:
        mannwhitney_results[column] = comparison_matrix_mannwhitney
        comparison_matrix_mannwhitney.to_csv(f"[EXP1]mannwhitney_{column}.csv")
    ...
    wilcoxon_matrix.to_csv(f"[EXP1]wilcoxon_{column}.csv")
    ...
    plt.tight_layout()
    plt.savefig(f"[EXP1]boxplot_{column}.png")
    plt.show()
...
for column, ttest_matrix in ttest_results.items():
    if ttest_matrix is not None:
        ttest_matrix.to_csv(f"[EXP1]ttest_{column}.csv")

for column, mannwhitney_matrix in mannwhitney_results.items():
    if mannwhitney_matrix is not None:
        mannwhitney_matrix.to_csv(f"[EXP1]mannwhitney_{column}.csv")

# Salvar análise estatística
analysis_df = pd.DataFrame(analysis_results)
analysis_df.to_csv("[EXP1]analise_estatistica.csv", index=False)
