...
data_path = "exp3.csv"  # Substitua pelo caminho correto
df = pd.read_csv(data_path, sep=';', decimal=',')

# Selecionar variáveis para análise
numeric_columns = ['Acurácia', 'Precision', 'atributos','Recall','F1_Score','AUC_ROC','treinamento (min)']  # Variáveis de interesse
...
custom_order = [
    ...
    "LASSO", "TSNE", "UMAP",
    "PCA", "RANDOM SEARCH",  "LAPPAS & YANNACOPOULOS", "[EFS-AHP]GPT"
...
for column in numeric_columns:
    ...
    if follows_normal:
        ...
        comparison_matrix_ttest.to_csv(f"[EXP3]ttest_{column}.csv")
    else:
        mannwhitney_results[column] = comparison_matrix_mannwhitney
        comparison_matrix_mannwhitney.to_csv(f"[EXP3]mannwhitney_{column}.csv")
    ...
    wilcoxon_matrix.to_csv(f"[EXP3]wilcoxon_{column}.csv")
    ...
    plt.tight_layout()
    plt.savefig(f"[EXP3]boxplot_{column}.png")
    plt.show()
...
for column, ttest_matrix in ttest_results.items():
    if ttest_matrix is not None:
        ttest_matrix.to_csv(f"[EXP3]ttest_{column}.csv")

for column, mannwhitney_matrix in mannwhitney_results.items():
    if mannwhitney_matrix is not None:
        mannwhitney_matrix.to_csv(f"[EX3]mannwhitney_{column}.csv")

# Salvar análise estatística
analysis_df = pd.DataFrame(analysis_results)
analysis_df.to_csv("[EXP3]analise_estatistica.csv", index=False)
