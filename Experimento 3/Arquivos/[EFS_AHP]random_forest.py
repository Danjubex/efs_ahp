...
# Este vetor deve ser o resultado do cálculo AHP e refletir as prioridades dos grupos
ahp_vector = [0.0089, 0.0131, 0.0226, 0.0533, 0.0533, 0.0533, 0.0533, 0.0533, 0.0533, 0.0533, 0.0533, 0.0533, 0.0533, 0.1402, 0.1413, 0.1413]
...
csv_file = "AG_BOT_RESULTS_EFS_AHP_EXP3.csv"
moeda = 'GOLD'
tipo  = 'COMPRA'
...
def select_features_by_group(individual, group_selection):
    selected_features = []
...
def group_selection_based_on_ahp():
    selected_groups = []
    for priority in ahp_vector:       
        random_number = random.uniform(0, 1)     
        if random_number < priority:
            selected_groups.append(1)
        else:
            selected_groups.append(0)
    return selected_groups

def feature_selection_within_selected_groups(selected_groups):
    selected_features = []
    group_names = list(GROUPS.keys())
    for i, group_selected in enumerate(selected_groups):
        if group_selected == 1:
            group_name = group_names[i]
            group_features = GROUPS[group_name]
            num_features_to_select = random.randint(1, len(group_features))
            selected_features.extend(random.sample(group_features, num_features_to_select))
    return selected_features
...
def evaluate_individual_by_group(individual, X, y, X_val, y_val, cv_folds=5, best_params=None):
    ...
    selected_groups = group_selection_based_on_ahp()
    selected_features = feature_selection_within_selected_groups(selected_groups)
...
if __name__ == "__main__":
    ...
    #TESTE DOS ÚLTIMOS 60 
    X_test, y_test = load_data_from_json("[ULTIMOS_60]GOLD.json")
...
# Balanceia as classes no conjunto de treino, aumentando a representatividade das classes minoritárias.
# Regularização L1 e L2:

# Adiciona duas camadas de regularização:
# L1: Lasso, usada para selecionar features mais importantes.
# L2: Ridge, usada para reduzir o overfitting sem eliminar features.
# Normalização:

# Normaliza os dados de treino, validação e teste para melhorar a performance de algoritmos sensíveis a escalas.
...
# Proporção de previsões corretas sobre o total de instâncias. O modelo acertou a classificação de aproximadamente 91% dos exemplos, indicando boa capacidade geral.
# Precisão (99.39%):

# Alta precisão mostra que o modelo raramente comete erros ao classificar uma instância como "ganhou". A taxa de falsos positivos é muito baixa.
# Recall (90.97%):

# O modelo consegue identificar cerca de 91% dos exemplos positivos reais. Isso indica que há falsos negativos, mas o recall ainda é satisfatório.
# F1-Score (94.99%):

# Um equilíbrio entre precisão e recall. Isso sugere que o modelo está bem ajustado para lidar com classes desbalanceadas.
# AUC-ROC (97.69%):

# O modelo tem excelente capacidade de discriminar entre as classes "perdeu" e "ganhou". Um AUC próximo de 1 é uma forte evidência de desempenho robusto.
# Matriz de Confusão:

# yaml
# Copiar código
# [[ 164    8]   -> Verdadeiros Negativos: 164 | Falsos Positivos: 8
#  [ 129 1299]]  -> Falsos Negativos: 129 | Verdadeiros Positivos: 1299
# Classe "perdeu":
# 164 casos corretamente classificados como "perdeu".
# Apenas 8 instâncias incorretamente classificadas como "ganhou".
# Classe "ganhou":
# 1299 instâncias corretamente classificadas.
# 129 casos incorretamente classificados como "perdeu".
# Insights:
# A precisão é muito alta (99.39%), mas há espaço para melhorar o recall (90.97%). Isso pode ser ajustado dependendo do objetivo do modelo:

# Se os falsos negativos forem críticos (ex.: prever "ganhou" corretamente), ajustes no threshold de decisão ou maior foco no recall podem ser necessários.
# Se os falsos positivos forem mais críticos, o modelo já está adequado, considerando a alta precisão.
# Falsos negativos:

# O modelo deixou de identificar 129 instâncias reais de "ganhou". Pode ser útil analisar essas instâncias para identificar padrões e ajustar as features ou o balanceamento.
# A robustez do modelo é indicada pela consistência entre validação e teste. A diferença marginal entre as métricas nos dois conjuntos reforça a generalização do modelo.

# Recomendações:
# Ajuste do Threshold de Decisão:

# Mova o limiar de decisão do modelo para melhorar o recall, caso necessário.
# Análise dos Erros:

# Analise os falsos negativos e falsos positivos para identificar padrões que o modelo possa não estar capturando adequadamente.
# Balanceamento:

# Experimente outras estratégias de balanceamento além do SMOTE, como técnicas híbridas (over + undersampling), para reduzir o impacto de exemplos excessivamente representados.
# Feature Importance:

# Analise a importância das features para identificar as que mais contribuem para o desempenho e possíveis melhorias.
...
# [BD]GOLD.db Distribuição de 'perdeu_ganhou' antes do mapeamento:
# perdeu_ganhou
# GANHOU    157214
# PERDEU     42659
# Name: count, dtype: int64
# Tamanho do conjunto de treinamento: 4800 amostras
# Tamanho do conjunto de teste: 1600 amostras
# Distribuição de classes após SMOTE:
# 0    25581
# 1    25581
# Name: count, dtype: int64
# Dados normalizados para validação e teste
...
# Melhor acurácia: 0.8826474401335511
