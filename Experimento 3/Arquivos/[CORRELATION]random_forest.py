...
def perform_correlation_feature_selection_with_grid_search(X, y, param_grid, output_file, run_id, random_state):
    ...
    # Limiares de correlação a serem testados
    #correlation_thresholds = [0.0, 0.1, 0.15, 0.2, 0.25, 0.3, 0.4, 0.5]#MELHOR 0.1
    correlation_thresholds = [0.1]

    for threshold in correlation_thresholds:
        print(f"\nTestando com limiar de correlação: {threshold}")
        
        # Seleção de features usando análise de correlação
        feature_correlations = pd.DataFrame(X_train_bal).corrwith(pd.Series(y_train_bal))
        selected_features = feature_correlations[feature_correlations.abs() > threshold].index.tolist()
        print(f"Número de features selecionadas com limiar {threshold}: {len(selected_features)}")
        
        if len(selected_features) == 0:
            print("Nenhuma feature selecionada com esse limiar. Pulando para o próximo.")
            continue
        ...
        print(f"Resultados da Rodada {run_id} - Limiar {threshold}:")
        ...
        with open(output_file, mode='a', newline='') as file:
            ...
            writer.writerow([
                run_id, threshold, len(selected_features), grid_search.best_score_, val_accuracy, test_accuracy,
...
if __name__ == "__main__":
    ...
    output_file = "AG_DOUTORADO_RESULTS_CORRELATION_FEATURE_SELECTION_EXP3.csv"

    # Criar ou abrir o arquivo CSV
    with open(output_file, mode='w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow([
            "Run_ID", "Threshold", "Num_Selected_Features", "CrossVal_Accuracy", "Validation_Accuracy",
    ...
    for run_id in range(1, 11):
        ...
        perform_correlation_feature_selection_with_grid_search(
