...
def perform_umap_with_grid_search(X, y, n_components_list, param_grid, output_file):
    ...
    # Iterar sobre os valores de componentes
    for component in n_components_list:
        
        random_state = np.random.randint(0, 10000) 
        print(f"Testando UMAP com {component} componentes...")

        # Aplicar UMAP ao conjunto de treinamento, validação e teste
        umap = UMAP(n_components=component, random_state=42)
        X_train_umap = umap.fit_transform(X_train_bal)
        ...
        grid_search.fit(X_train_umap, y_train_bal)
        ...
        final_model.fit(X_train_umap, y_train_bal)

        # Avaliar o modelo no conjunto de validação
        val_predictions = final_model.predict(X_val_umap)
        val_probabilities = final_model.predict_proba(X_val_umap)[:, 1]
        ...
        test_predictions = final_model.predict(X_test_umap)
        test_probabilities = final_model.predict_proba(X_test_umap)[:, 1]
        ...
        print(f"Validação com {component} componentes UMAP:")
...
if __name__ == "__main__":
    ...
    n_components_list = [2, 3, 5, 10, 15, 20]#5 melhor
    ...
    output_file = "AG_DOUTORADO_RESULTS_UMAP_EXP3.csv"

    # Executar o teste com UMAP e grid search
    perform_umap_with_grid_search(X, y, n_components_list, param_grid, output_file)
