...
csv_file = "AG_DOUTORADO_RESULTS_RANDOM_EXP3.csv"
moeda = 'GOLD'
tipo  = 'COMPRA'
...
def feature_selection_within_selected_groups(selected_groups):
    """
    Seleciona features aleatoriamente, ignorando os grupos.

    Args:
        selected_groups (List[int]): Lista binária representando os grupos (não utilizada na seleção aleatória).

    Returns:
        List[int]: Lista de índices das features selecionadas aleatoriamente.
    """
    total_features = sum(len(features) for features in GROUPS.values())  # Conta o total de features disponíveis.
    num_features_to_select = random.randint(1, total_features)  # Define quantas features serão selecionadas.
    selected_features = random.sample(range(total_features), num_features_to_select)  # Seleciona features aleatoriamente.
...
def evaluate_individual_by_group(individual, X, y, cv_folds=5, best_params=None):
    ...
    # Treinar e avaliar o modelo com as features selecionadas
    ...
    scores = cross_val_score(model, X[:, selected_features], y, cv=cv_folds, scoring='accuracy')#accuracy
    return (np.mean(scores),)


    
def create_toolbox_group_selection(X, y, num_features,best_params=None):
    ...
    toolbox.register("evaluate", evaluate_individual_by_group, X=X, y=y,cv_folds=5,best_params=best_params)
...
def genetic_algorithm(X_train, y_train, X_val, y_val, X_test, y_test, num_features, n_gen=50, pop_size=10, cx_prob=0.9, mut_prob=0.05, elitism_size=5, best_params=None):
    ...
    for cycle in range(1, 11):  # Executar 10 ciclos
        ...
        toolbox = create_toolbox_group_selection(X_val, y_val, num_features, best_params)
        ...
        for gen in range(n_gen):
            ...
            val_probabilities = model.predict_proba(X_val[:, selected_features])[:, 1]  # Apenas probabilidade para a classe positiva

            val_accuracy = accuracy_score(y_val, val_predictions)
            val_precision = precision_score(y_val, val_predictions, zero_division=0)
            val_recall = recall_score(y_val, val_predictions)
            val_f1 = f1_score(y_val, val_predictions)
            val_roc_auc = roc_auc_score(y_val, val_probabilities)  # Probabilidades para a classe positiva
            ...
            with open(csv_file, mode='a', newline='') as file:
                writer = csv.writer(file)
                writer.writerow([cycle, gen + 1, len(selected_features), selected_features, val_accuracy, val_precision, val_recall, val_f1, val_roc_auc, training_time_minutes])
...
if __name__ == "__main__":
    ...
    print("Distribuição de classes após UNDERSAMPLE:")
